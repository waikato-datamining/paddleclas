...
* Pull and run image (adjust volume mappings `-v`):
  ...
  docker run --shm-size 8G --net=host \
    ...
    -it public.aml-repo.cms.waikato.ac.nz:443/paddle/paddleclas:2.6.0-2_cpu
  ```
...
* Pull and run image (adjust volume mappings `-v`):
  ...
  docker run --shm-size 8G --net=host \
    -v /local/dir:/container/dir \
    -it waikatodatamining/paddleclas:2.6.0-2_cpu
  ```

### Build local image

* Build the image from Docker file (from within /path_to/paddleclas/2.6.0-2_cpu)
...
* Run the container
  ...
  ```bash
  docker run --shm-size 8G --net=host -v /local/dir:/container/dir -it paddleclas
  ```
...
```bash
docker build -t paddleclas:2.6.0-2_cpu .
```
...
* Tag
  ...
  ```bash
  docker tag \
    paddleclas:2.6.0-2_cpu \
    public-push.aml-repo.cms.waikato.ac.nz:443/paddle/paddleclas:2.6.0-2_cpu
  ```
  
* Push

  ```bash
  docker push public-push.aml-repo.cms.waikato.ac.nz:443/paddle/paddleclas:2.6.0-2_cpu
  ```
...
* Tag
  ...
  docker tag \
    paddleclas:2.6.0-2_cpu \
    waikatodatamining/paddleclas:2.6.0-2_cpu
  ```
  
* Push

  ```bash
  docker push waikatodatamining/paddleclas:2.6.0-2_cpu
  ```
  If error "no basic auth credentials" occurs, then run (enter username/password when prompted):
