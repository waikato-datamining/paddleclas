...
Uses PyTorch 1.11.0, CPU and PaddleClas 2.6.0.
...
* Pull and run image (adjust volume mappings `-v`):
  ...
  docker run --shm-size 8G --net=host \
    -v /local/dir:/container/dir \
    -it waikatodatamining/paddleclas:2.6.0_cpu
  ```

### Build local image

* Build the image from Docker file (from within /path_to/paddleclas/2.6.0_cpu)
...
```bash
docker build -t paddleclas:2.6.0_cpu .
```
...
* Push
  ...
  ```bash
  docker push waikatodatamining/paddleclas:2.6.0_cpu
  ```
  If error "no basic auth credentials" occurs, then run (enter username/password when prompted):
